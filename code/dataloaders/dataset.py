...
class ACDC(Dataset):
    def __init__(self, base_dir=None, split='train', num=None, transform=None):
        self._base_dir = base_dir
        # self._base_dir = r'D:\download\data\ACDC_challenge_20170617'
        self.sample_list = []
        self.split = split
        self.transform = transform
        if self.split == 'train':
...
class Pancreas(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        image = h5f['image'][:]
        label = h5f['label'][:]
        # nifti_img = nib.load(self._base_dir + "/images/" + image_name + ".nii.gz")
        # image = nifti_img.get_fdata()
        # nifti_label = nib.load(self._base_dir + "/labels/" + image_name + ".nii.gz")
