...
def var_all_case(model, num_classes, patch_size=(112, 112, 80), stride_xy=18, stride_z=4, dataset_name="LA"):
    if dataset_name == "LA":
        with open('E:/PyCharmProjects/UA-MT-master/data/test.list', 'r') as f:
            image_list = f.readlines()
        image_list = ["E:/PyCharmProjects/UA-MT-master/data/2018LA_Seg_Training Set/" + item.replace('\n', '') + "/mri_norm2.h5" for item in
                      image_list]
    elif dataset_name == "Pancreas_CT":
        with open('E:/PyCharmProjects/UA-MT-master/data/Pancreas_h5/test.list', 'r') as f:
            image_list = f.readlines()
        image_list = ["E:/PyCharmProjects/UA-MT-master/data/Pancreas_h5/" + item.replace('\n', '') + "_norm.h5" for item in image_list]
    elif dataset_name == "ACDC":
        with open('D:\download\data\ACDC_challenge_20170617/test.list', 'r') as f:
            image_list = f.readlines()
        image_list = ["D:/download/data/ACDC_challenge_20170617/data/" + item.replace('\n', '') + ".h5" for item
                      in image_list]
    elif dataset_name == "bra_2019":
        # 分为 训练集 验证集 测试集
        with open('D:\YH\mynet\data\BraTS2019/val.txt', 'r') as f:
            image_list = f.readlines()
        image_list = ["D:\YH\mynet\data\BraTS2019/data/" + item.replace('\n', '') + ".h5" for item
                      in image_list]
    ...
    for image_path in loader:
        ...
        if dataset_name == "ACDC":
            prediction, score_map = test_single_case_acdc(model, image, stride_x=stride_xy, stride_yz=stride_z, patch_size=patch_size,
        ...
        if np.sum(prediction) == 0:
            ...
            # prediction shape[0] = 3
            for class_num in range(1, num_classes):
                dice += metric.binary.dc(prediction[class_num], label == class_num)
            dice /= (num_classes - 1)
        else:
...
def test_single_case_acdc(model, image, stride_x, stride_yz, patch_size, num_classes=1):
    w, h, d = image.shape
    ...
    sy = math.ceil((hh - patch_size[1]) / stride_yz) + 1
    sz = math.ceil((dd - patch_size[2]) / stride_yz) + 1
    ...
    for x in range(0, sx):
        ...
        for y in range(0, sy):
            ys = min(stride_yz * y, hh - patch_size[1])
            for z in range(0, sz):
                zs = min(stride_yz * z, dd - patch_size[2])
                ...
                y = y.cpu().data.numpy()
                for class_num in range(1, num_classes):
                    score_map[class_num, xs:xs + patch_size[0], ys:ys + patch_size[1], zs:zs + patch_size[2]] \
                        = score_map[class_num, xs:xs + patch_size[0], ys:ys + patch_size[1], zs:zs + patch_size[2]] + y[0, class_num, :, :, :]
                    cnt[class_num, xs:xs + patch_size[0], ys:ys + patch_size[1], zs:zs + patch_size[2]] \
                        = cnt[class_num, xs:xs + patch_size[0], ys:ys + patch_size[1], zs:zs + patch_size[2]] + 1

    label_map = np.zeros((num_classes,) + image.shape).astype(np.float32)
    for class_num in range(1, num_classes):
        score_map[class_num] = score_map[class_num] / cnt[class_num]
        label_map[class_num] = (score_map[class_num] > 0.5).astype(np.int)
    if add_pad:
