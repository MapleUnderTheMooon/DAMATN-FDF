...
#
#     def initialize(self, modules):
#         """torch.nn.init.kaiming_normal_(m.weight) 初始化参数 权重"""
#         for m in modules:
#             if isinstance(m, nn.Conv2d):
#                 torch.nn.init.kaiming_normal_(m.weight)
#             elif isinstance(m, nn.BatchNorm2d):
...
class Decoder_Attention(nn.Module):
    def __init__(self, num_classes, low_level_inplanes,  kernel_size=3, padding=1, attention_mode='CBAM'):
        super().__init__()
        ...
        if attention_mode == 'CBAM':
            ...
            self.attention = PAM_Module(128)
        else:
            self.attention = ChannelAttention(128)

        self.initialize([self.attention])
...
class CBAM(nn.Module):
    ...
    def __init__(self, in_planes, ratio=16, kernel_size=7):
        ...
        #
        # self.ca = PAM_Module(in_planes)
        # self.sa = ChannelAttention(in_planes, ratio)
